...
async def get_user_notifications(
    ...
    results = await db.execute(select(models.Notification).where(models.Notification.user_id == user_id).offset(offset).limit(limit))
...
async def get_notification(db: AsyncSession, id: UUID):
...
async def mark_notification_as_read(db: AsyncSession, id: UUID):
